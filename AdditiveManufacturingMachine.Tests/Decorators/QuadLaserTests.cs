...
namespace AdditiveManufacturingMachine.UnitTests.Decorators
{
    ...
    internal class QuadLaserTests
    {
        ...
        {
            QuadLaser quadLaser = new QuadLaser(_mockMachine.Object);
            double cost = quadLaser.Cost;
            Assert.That(cost, Is.EqualTo(225_000));
        }
        ...
        {
            QuadLaser quadLaser = new QuadLaser(_mockMachine.Object);
            string description = quadLaser.Description;
            Assert.That(description, Is.EqualTo(" with Quad Laser"));
        }

        [Test(Author = "MM")]
        public void Should_throw_ArgumentNullException_if_machine_is_null()
        {
            Assert.That(() => new QuadLaser(null), Throws.ArgumentNullException);
        }
    }
}
